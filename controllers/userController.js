...
const userControl = {
    ...
    changePrdtQty: async (req, res) => {
        try {
            ...
            console.log('366');
                ...
                console.log('369 ',count);
                console.log('369 ',cart);
                // const newCart = await CartModel.findOne({userId: userId, 'products.item': ObjectId(productId)})
                console.log('372 ',req.body);
                console.log('373 ',req.body.pid);
                console.log('373 ',req.body.product);
                
                if (count == 1) {
                    console.log('374');
                    let updateProduct = await ProductsModel.updateOne({ _id: Object(req.body.product) }, { $inc: { stock: 1 } })
                    console.log('375 = ',updateProduct);
                    res.json({ quantity: parseInt(quantity) + 1, removeProduct: false });
                }
                else if (count == -1) {
                    updateProduct = await ProductsModel.updateOne({ _id: Object(req.body.product) }, { $inc: { stock: -1 } })
                    console.log('380 = ',updateProduct);
                    res.json({ quantity: parseInt(quantity) - 1, removeProduct: false })
                }
            // console.log('379');
    ...
    //     let products = await ProductsModel.find({title: {$regex: req.body.searchKey}})
    //     console.log('product d= ',products);
    // }
}
