...
const adminLoginCheck = (req, res, next) => {
  ...
  else if (!req.session.admin) res.redirect("/");
  else next();
};

const forLogin = (req, res, next) => {
  if (req.session.loggedIn && req.session.admin) res.redirect("/admin");
  else if (req.session.loggedIn && !req.session.admin) res.redirect("/");
  else next();
};
...
router.post(
  "/add-product",
  adminLoginCheck,
  uploadPrdt.array("image1", 4),
  adminControl.addProductsPost,
);
...
router.post(
  "/change-order-status",
  adminLoginCheck,
  adminControl.changeOrderStatus,
);
router.get(
  "/view-order-products",
  adminLoginCheck,
  adminControl.viewOrderedProduct,
);
...
router.get(
  "/add-product-offer",
  adminLoginCheck,
  adminControl.addProductOfferGet,
);
router.post(
  "/add-product-offer",
  adminLoginCheck,
  adminControl.addProductOfferPost,
);
router.get(
  "/edit-offer-product",
  adminLoginCheck,
  adminControl.editOfferProductsGet,
);
router.patch(
  "/edit-product-offer",
  adminLoginCheck,
  adminControl.editOfferProductsPatch,
);
router.delete(
  "/delete-product-offer",
  adminLoginCheck,
  adminControl.deleteOfferProduct,
);
...
const uploadMultipleFiled = uploadImage.fields([
