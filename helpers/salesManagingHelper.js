  ...
  monthlyReport: () => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let orderSuccess = await OrdersModel.find({
        ...
        let orderTotal = await OrdersModel.find({
          date: { $gte: start, $lte: end },
        });
        ...
        for (let i = 0; i < orderSuccessLength; i++) {
          ...
          if (orderSuccess[i].product.productOfferActive) {
            total =
              total -
              (parseInt(orderSuccess[i].product.price) / 100) *
                parseInt(orderSuccess[i].product.productOffer.offerPercentage);
          }
        ...
        let productCount = await OrdersModel.find({
          status: { $nin: ["cancelled"] },
          date: { $gte: start },
        }).count();
  ...
  getReport: (startDate, endDate) => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        productCount = productCount = await OrdersModel.find({
          status: { $nin: ["cancelled"] },
          date: { $gte: start },
        }).count();
        var data = {
  ...
  dailyReport: () => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        productCount = await OrdersModel.find({
          status: { $nin: ["cancelled"] },
          date: { $gte: start },
        }).count();
